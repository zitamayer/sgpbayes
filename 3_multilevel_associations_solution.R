...
# cross-lagged model
load("crosslag.goalmissing.goalimportance.smaller.Rdata")
...
######################################################################################################
##           LEVEL-2 PREDICTOR: BETWEEN-PERSON DIFFERENCES IN INTERCEPTS AND SLOPES                 ##
######################################################################################################
...
# regressed on itself at last time point (autoregression) and goal missing at the last time point (cross-lag)
...
# regressed on itself at last time point (autoregression) and goal importance at the last time point (cross-lag)
f2 <- bf(goalmissing ~ goalmissing_lag + goalimportance_lag + (goalmissing_lag | id))

# crosslag.goalmissing.goalimportance.smaller <- brm(f1 + f2 + set_rescor(TRUE), # combines two defined models into one multivariate model, with correlated residuals of DVs
#                                         data = dt_lags,
#                                         iter = 2000,
#                                         warmup = 1000,
#                                         chains = 2,
